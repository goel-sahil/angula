...
export class StudentListingComponent implements AfterViewInit {
  displayedColumns: string[] = ['id', 'firstname', 'lastname', 'class', 'subject', 'marks'];
...
function createNewUser(id: number): UserData {
  ...
  return {
    ...
    lastname: name,
    class: name,
    subject: Math.round(Math.random() * 100).toString(),
    marks: FRUITS[Math.round(Math.random() * (FRUITS.length - 1))]
  };
}
